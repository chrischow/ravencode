...
# There is some inherrent error that is fixed with the stopgap below. Something wrong with the polyfill configuration.
sed -E 's/\(([a-zA-Z]{1,2}\.element)\.remove\(\)/(\1.parentNode.removeChild(\1)/' \
    "./build/index-fixed.html" > "./build/index.html"
rm ./build/index-fixed.html

# mv ./build/index-fixed.html ./build/index.html

# code to fix the ttf file and it's reference to txt.
# sed -E 's/(static\/media\/codicon\..*)\.ttf/\1.txt/' "./build/index-fixed.html" > "./build/index.html"
# rm ./build/index-fixed.html
